...
public class TwitterStreamTask {
     ...
     */
    private static final Class[] KRYO_CLASSES = ImmutableList.builder().add(GeoLocation.class).add(Status.class).add(User.class).build()
            .toArray(new Class[] {});
     ...
     * line. In our example we will call the task from a WordCountTest instead. See
    ...
    public void run() throws InterruptedException {
         ...
         */
        SparkConf conf = new SparkConf().setMaster("local[*]").set("spark.serializer", "org.apache.spark.serializer.KryoSerializer")
        ...
        streamingContext.receiverStream(new TwitterReceiver(StorageLevel.MEMORY_ONLY()))
                .foreachRDD(rdd -> rdd.coalesce(10).foreach(message -> LOGGER.info(message.getText())));
